...
/// assert_ne!(e1, e2);
/// ```
pub struct DefaultRandomCoin<H>
where
...
impl<H: Hasher + ElementHasher> DefaultRandomCoin<H> {
...
impl<B: StarkField, H: Hasher + ElementHasher<BaseField = B>> RandomCoin for DefaultRandomCoin<H> {
    type BaseField = B;
    type Hasher = H;
